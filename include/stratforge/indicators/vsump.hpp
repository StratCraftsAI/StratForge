...
/// VSumP: sum of volume on up-bars over the trailing window.
/// Alpha158 formula: Sum($volume * ($close > Ref($close,1)), N)
class VSumP : public Indicator<VSumP> {
public:
    explicit VSumP(const Line<double>& close, const Line<double>& volume, std::size_t period)
