...
/// VSumN: sum of volume on down-bars over the trailing window.
/// Alpha158 formula: Sum($volume * ($close < Ref($close,1)), N)
class VSumN : public Indicator<VSumN> {
public:
    explicit VSumN(const Line<double>& close, const Line<double>& volume, std::size_t period)
